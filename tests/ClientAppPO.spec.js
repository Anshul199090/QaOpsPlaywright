...
import dataSet1 from '../utils/placeorderTestData1.json' assert { type: 'json' };
...
for (const data of dataSet) {
    test(`@WebJs Client app end 2 end for ${data.prodName}`, async ({ page }) => {
...
test(`@UIJs Client app end 2 end for new ${data.prodName}`, async ({ page }) => {
    //const context = await browser.newContext();
    //const page = await context.newPage();
